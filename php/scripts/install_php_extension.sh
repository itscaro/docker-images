	...
	do
		...
		then
			...
			case "${PHP_MODULE_TO_INSTALL}" in
			    amqp)
			        if test ${PHP_MAJMIN_VERSION} -ge 703
			        then
                        # Temporary workaround until amqp > 1.9.3 is relesed
                        MODULE_SOURCE=https://github.com/pdezwart/php-amqp/archive/master.tar.gz
                        cd "$(getPackageSource https://github.com/pdezwart/php-amqp/archive/master.tar.gz)"
                        phpize
                        ./configure
                        make
                        make install
